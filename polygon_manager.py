...
def draw_regions(camera_no, polygons_dict: dict, polygons_file_name="polygons_config.pkl"):
    _polygons_dict = load_pickle(polygons_file_name)
    if camera_no in _polygons_dict.keys():
        image = _polygons_dict[camera_no]['image']
...
def get_aisle_regions(config):
    aisle_regions = {}
    crop_regions = {}
    for stream in config['input_streams']['sources']:
...
if __name__ == '__main__':
    ...
    aisle_regions, crop_regions = get_aisle_regions(config)
    for cam_no in aisle_regions:
        image = draw_regions(cam_no, aisle_regions[cam_no])
        image = draw_regions(cam_no, crop_regions[cam_no])
        cv2.imshow(f"regions_{cam_no}", image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()